...
parameters = {'kernel': ['linear','poly'], 'C': [0.01, 0.1, 1], 'tol':[1e-3,1e-2,1e-1]}

svc = svm.SVC(verbose = 0, decision_function_shape='ovo') #Binary classification (ovr)
cv = ShuffleSplit(n_splits=10, test_size=0.2, random_state=0
                  )
grid_search = GridSearchCV(svc, parameters, cv=3)
grid_search.fit(trainImages, trainLabels)

random_search = RandomizedSearchCV(svc, param_distributions=parameters,n_iter = 10, cv=3)
random_search.fit(trainImages,trainLabels)

classifier = svm.SVC(gamma='auto', verbose = 1, kernel='poly',decision_function_shape='ovr') #Builds SVM model using a polynomial kernel
